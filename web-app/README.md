...
유효한 긴 입력은 경험·묘사 2회, 롤플레이 4회가 기본입니다. 선택한 초안에서 원문에 없는 숫자값·부호가 발견되거나 보조 카드의 형식·언어 검증에 실패하면, 각 단계에서 한 번만 수정 요청합니다. 경험·묘사는 최대 4회, 롤플레이는 최대 6회이며 재검증 실패는 오류로 반환합니다. 숫자 검사는 아라비아 숫자값만 비교하므로 영어로 쓴 수량, 단위, 감정 등 의미 전체를 검증하지는 못합니다.
...
정보 추가·30분→20분 정정·카페 묘사·카페 롤플레이·도서관 묘사·호텔 롤플레이의 여섯 입력을 순서대로 실행합니다. 마지막 두 입력은 프롬프트 예시와 다른 내용으로 일반화를 확인합니다. `--output /원하는/새파일.json`으로 입력과 결과를 저장할 수 있으며 기존 파일은 덮어쓰지 않습니다. 예제별 핵심 표현·한국어 뼈대·일부 원문에 없는 표현을 확인하는 제한된 패턴 검사입니다. 통과가 의미·문법 검증 완료를 뜻하지 않고, 정상적인 바꿔 말하기를 누락으로 볼 수도 있습니다. 웹 요청 시 실행하는 검사는 아닙니다.
