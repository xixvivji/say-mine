...
정보 추가·30분→20분 정정·묘사·롤플레이 네 입력을 순서대로 실행합니다. `--output /원하는/새파일.json`으로 입력과 결과를 저장할 수 있으며 기존 파일은 덮어쓰지 않습니다. 예제별 핵심 표현과 한국어 뼈대를 확인하는 제한된 패턴 검사입니다. 통과가 의미·문법 검증 완료를 뜻하지 않고, 정상적인 바꿔 말하기를 누락으로 볼 수도 있습니다. 웹의 생성 횟수에는 영향을 주지 않습니다.
